...
def getFmeasure(original_img, test_img, width, height):
    DP = 0
    YP = 0
    YN = 0
    for i in range(height):
        for j in range(width):
            if original_img[i][j] == 255 and test_img[i][j] == 255:
                DP += 1
            if original_img[i][j] == 0 and test_img[i][j] == 255:
                YP += 1
            if original_img[i][j] == 255 and test_img[i][j] == 0:
                YN += 1
    if YP==0:
        YP=1
